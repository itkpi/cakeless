...
object LifecycleExample extends IOApp {
  def run(args: List[String]): IO[ExitCode] = {
    val comp1 = cakeDelayed[IO, AllComponents1 with ExecutionContextComponent with FileConfigComponent]
      .preStart {
    ...
    val comp2 =
      cakeSingleton[IO, AllComponents2 with ExecutionContextComponent with PropsComponent with DbComponent]
        .postStartUseF { comp =>
